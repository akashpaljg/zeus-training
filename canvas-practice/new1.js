...
getCumulativeHeightTillRow(startRow){
    ...
    drawHeader(visibleArea = null) {
        ...
        for (let index = startCol; index < endCol; index++) {
            ...
            // let width = this.visibleArea.startCol < 0 ? 0: this.getCumulativeWidthTillColumn(index);
...
getVisibleArea(scrollX, scrollY) {
    ...
    // Find startCol
    for (let i = 0; i < this.headers.length; i++) {
        if (accumulatedWidth > scrollX) {
            startCol = Math.max(0, i - 1);
            break;
        }
        accumulatedWidth += this.headers[i].width || 100;
    }
    ...
    for (let i = 0; i < this.headers.length; i++) {
        ...
        if (accumulatedWidth > scrollX + this.container.clientWidth) {
            endCol = Math.min(i + 1, this.headers.length - 1);
            break;
        }
    }
